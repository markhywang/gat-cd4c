    ...
    def _eval_model(self, percent_data, test_dataset: DrugProteinDataset, batch_size: int = 50) \
        ...
        test_loader = DataLoader(
            ...
            collate_fn=lambda b: collate_drug_prot(
                b, hard_limit=80, drug_edge_feats=17, cross_cutoff=5.0
            )
        )
        ...
        with torch.no_grad():
            for batch_num, batch in enumerate(test_loader):
                # collate_drug_prot now returns (10-tuple):
                #   drug_ns [B,H,Fn], drug_es [B,H,H,Fe], drug_as [B,H,H]   — dense
                #   prot_ns [N_total,Fn], prot_eis [2,E], prot_eas [E,Fe],
                #   prot_batch [N_total]                                     — sparse
                #   cross_ei [2,E_cross], cross_ea [E_cross,1]               — sparse
                #   labels [B]
                ...
                # edges (cross_ei, cross_ea) before this path is functional.
                preds = self.model(d_n, d_e, d_a, p_n, p_ei, p_ea, p_batch, cross_ei, cross_ea).squeeze(-1)
...
class MoleculeViewer(tk.Frame):
    ...
    def _update_display(self) -> None:
        ...
        mol = dm.mol
        self._draw_molecule(mol, node_contributions, dm)
    ...
    def _get_node_contributions(self, idx: int) -> tuple[list[float], float]:
        ...
        # 1) Unpack full dataset entry (9-tuple from DrugProteinDataset.__getitem__)
        d_n, d_e, d_a, d_pos, p_n, p_e, p_i, p_pos, lbl = self.dataset[idx]
        # d_n:   [N_drug, F_n]          drug node features (padded)
        # d_e:   [N_drug, N_drug, F_e]  drug edge features
        # d_a:   [N_drug, N_drug]       drug adjacency
        # d_pos: [N_drug, 3]            drug atom positions (RDKit placeholder)
        # p_n:   [N_prot, F_n']         protein node features (sparse, no padding)
        # p_e:   [E_prot, F_e']         protein edge attrs (COO)
        # p_i:   [2, E_prot]            protein edge index (COO)
        # p_pos: [N_prot, 3]            protein Cα positions
        # lbl:   scalar
        # NOTE: protein is manually densified below for the unchanged model.forward.
        # TODO: remove densification once DualGraphAttentionNetwork supports sparse protein.
        ...
        return node_contributions, real_pred, lbl.item()
