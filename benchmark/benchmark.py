...
def load_test_data(data_path: str, seed: int, frac_train: float, frac_validation: float,
    ...
    traing_df, remaining_df = train_test_split(data_df,
                                               text_size=frac_validation + frac_test,
...
def load_model(device, model_path, in_features=333, out_features=1, num_edge_features=16,
               hidden_size=64, num_layers=3, num_attn_heads=4, dropout=0.2, pooling_dim=128):
    """
...
def evaluate_model(model: nn.Module, test_loader: DataLoader, deveice: torch.device, huber_beta: float) -> tuple:
    ...
    binary_labels = (all_labels >= pchembl_scores).astype(int)
...
def main():
    ...
    parser.add_argument("--in_features", type=int, default=333, help="Input feature dimension")
    ...
    parser.add_argument("--pooling_dim", type=float, default=128, help="Dimension for global pooling")
